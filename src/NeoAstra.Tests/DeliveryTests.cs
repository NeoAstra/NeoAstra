...
public sealed class DeliveryTests
{
    ...
    public void BundleConfigurationAndStagingAreStrictDeterministicAndRedacted()
    {
        ...
        if (OperatingSystem.IsWindows()) { StringAssert.Contains(platformInput, "MinVersion=\"10.0.0.0\""); StringAssert.Contains(platformInput, "Publisher=\"CN=Unsigned, OID.2.25.311729368913984317654407730594956997722=1\""); StringAssert.Contains(platformInput, "runFullTrust"); }
    }
    ...
    public void BundleRejectsUndeclaredMissingForbiddenCollisionWrongAbiAndCrossHostSigning()
    {
        ...
        var uppercaseExecutable = Path.Combine(fixture.Publish, fixture.ExecutableFile.ToUpperInvariant());
        if (!File.Exists(uppercaseExecutable)) { File.WriteAllText(fixture.Configuration, File.ReadAllText(fixture.Configuration).Replace("\"source.cs\"", $"\"{fixture.ExecutableFile}\", \"{fixture.ExecutableFile.ToUpperInvariant()}\"")); File.Copy(Path.Combine(fixture.Publish, fixture.ExecutableFile), uppercaseExecutable); project = NeoProjectConfiguration.Load(fixture.Configuration); Assert.AreEqual("bundle_collision", Assert.ThrowsExactly<NeoToolException>(() => NeoBundleOrchestrator.Run(project, fixture.Request(Path.Combine(fixture.Root, "collision"), false))).Code); }
    }
    ...
    private sealed class BundleFixture : IDisposable
    {
        ...
        internal void WriteNativeIdentity() { var native = Path.Combine(Publish, NativeFile); File.WriteAllText(Path.Combine(Publish, "neoastra-native.json"), $"{{\"schemaVersion\":1,\"rid\":\"{Rid}\",\"file\":\"{NativeFile}\",\"sha256\":\"{Hash(native).ToLowerInvariant()}\",\"abiMajor\":1,\"abiMinor\":0}}\n"); }
