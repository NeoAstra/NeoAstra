        ...
        try
        {
            var run = RunStaAsync(() => NeoApplication.Run(
                ...
                async application =>
                {
                    ...
                    var window = application.CreateWindow(new NeoWindowOptions { IsVisible = false, HasDecorations = false, IsResizable = false, IsAlwaysOnTop = true, ShowInTaskbar = false });
                    Assert.IsFalse(window.HasDecorations);
                    Assert.IsFalse(window.IsResizable);
                    Assert.IsTrue(window.IsAlwaysOnTop);
                    Assert.IsFalse(window.ShowInTaskbar);
                    window.HasDecorations = true;
                    window.IsResizable = true;
                    window.IsAlwaysOnTop = false;
                    window.ShowInTaskbar = true;
                    Assert.IsTrue(window.HasDecorations);
                    ...
                    Assert.AreEqual(NeoWindowState.Fullscreen, window.State);
