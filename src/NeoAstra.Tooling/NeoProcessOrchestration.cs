...
internal static class NeoOriginPolicy
{
    internal static Uri ValidateDevelopmentUrl(string value, bool allowRemote)
    {
        if (!Uri.TryCreate(value, UriKind.Absolute, out var uri) || uri.Scheme is not ("http" or "https") ||
            uri.UserInfo.Length != 0 || uri.Fragment.Length != 0 || uri.HostNameType is UriHostNameType.Unknown or UriHostNameType.Dns)
        ...
        if (!loopback && !allowRemote) throw new NeoToolException("remote_development_origin", "The development URL must use exactly 127.0.0.1 or ::1 unless allowRemoteDevServer is explicitly enabled.");
        return uri;
    }
...
internal sealed record NeoProcessStart(string Label, NeoCommand Command, string WorkingDirectory,
    IReadOnlyDictionary<string, string> Environment, IReadOnlySet<string> SecretEnvironment, Action<string, bool> Log);
...
internal sealed class NeoChildProcess : INeoChildProcess
{
    ...
    {
        if (!Directory.Exists(start.WorkingDirectory)) throw new NeoToolException("process_working_directory", $"{start.Label} working directory does not exist.");
        ...
        foreach (var argument in executable.PrefixArguments) info.ArgumentList.Add(argument);
        foreach (var argument in start.Command.Arguments.Skip(1)) info.ArgumentList.Add(argument);
        foreach (var pair in start.Environment) info.Environment[pair.Key] = pair.Value;
        _process = new Process { StartInfo = info, EnableRaisingEvents = true };
        try { if (!_process.Start()) throw new NeoToolException("process_start", $"Unable to start {start.Label}."); }
        catch (Exception exception) when (exception is not NeoToolException) { _process.Dispose(); throw new NeoToolException("process_start", $"Unable to start {start.Label}; verify the configured executable."); }
    ...
    public async Task StopAsync(TimeSpan timeout)
    {
        ...
        {
            try { await _completion.WaitAsync(timeout).ConfigureAwait(false); return; }
            catch (TimeoutException) { throw new NeoToolException("process_teardown_timeout", "A child process tree did not terminate within the bounded shutdown period."); }
        }
        var started = Stopwatch.StartNew();
        if (!_process.HasExited)
        {
            var graceful = false;
            try { graceful = OperatingSystem.IsWindows() ? _process.CloseMainWindow() : NeoSignals.Kill(_process.Id, 15) == 0; }
            catch (InvalidOperationException) { }
            if (graceful)
            {
                var grace = timeout < TimeSpan.FromSeconds(2) ? timeout : TimeSpan.FromSeconds(2);
                try { await _completion.WaitAsync(grace).ConfigureAwait(false); return; }
        ...
        if (remaining <= TimeSpan.Zero) throw new NeoToolException("process_teardown_timeout", "A child process tree did not terminate within the bounded shutdown period.");
        try { await _completion.WaitAsync(remaining).ConfigureAwait(false); }
        catch (TimeoutException) { throw new NeoToolException("process_teardown_timeout", "A child process tree did not terminate within the bounded shutdown period."); }
    }

    public async ValueTask DisposeAsync()
    {
        try { await StopAsync(TimeSpan.FromSeconds(10)).ConfigureAwait(false); }
    ...
    private static async Task PumpAsync(StreamReader reader, bool error, NeoProcessStart start)
    {
        ...
        while (await reader.ReadAsync(buffer.AsMemory()).ConfigureAwait(false) is var read && read != 0)
        {
            for (var index = 0; index < read; index++)
            {
                ...
                if (line.Length < maximumLineCharacters) line.Append(buffer[index]); else truncated = true;
            }
        }
        if (line.Length != 0 || truncated) Emit();

        void Emit()
        {
            var value = line.ToString();
            foreach (var name in start.SecretEnvironment)
                if (start.Environment.TryGetValue(name, out var secret) && secret.Length != 0) value = value.Replace(secret, "[REDACTED]", StringComparison.Ordinal);
            if (truncated) value += " [output truncated]";
    ...
    internal static (string Path, IReadOnlyList<string> PrefixArguments) ResolveExecutable(string executable, string workingDirectory)
    {
        if (Path.IsPathFullyQualified(executable) || executable.Contains(Path.DirectorySeparatorChar) || executable.Contains(Path.AltDirectorySeparatorChar))
        {
            ...
            if (!File.Exists(path)) throw new NeoToolException("executable_missing", $"Configured executable '{executable}' was not found.");
            if (OperatingSystem.IsWindows() && Path.GetExtension(path) is ".cmd" or ".bat" or ".ps1") throw new NeoToolException("executable_shell_shim", "Shell scripts are not executed implicitly; configure an executable command array.");
            ...
            foreach (var extension in extensions)
                {
                    ...
                    if (!OperatingSystem.IsWindows() || candidate.EndsWith(".exe", StringComparison.OrdinalIgnoreCase) || candidate.EndsWith(".com", StringComparison.OrdinalIgnoreCase)) return (candidate, Array.Empty<string>());
                    var packageManager = ResolveWindowsPackageManager(executable, Path.GetDirectoryName(candidate)!);
                    if (packageManager is not null) return packageManager.Value;
                }
...
internal sealed class NeoReadinessProbe : INeoReadinessProbe
{
    public async Task WaitAsync(Uri url, TimeSpan timeout, CancellationToken cancellationToken)
    {
        ...
        while (true)
        {
            ...
            try
            {
                ...
                if (ValidateResponse(url, response)) return;
            }
            ...
            try { await Task.Delay(100, timeoutSource.Token).ConfigureAwait(false); }
    ...
    internal static bool ValidateResponse(Uri configuredUrl, HttpResponseMessage response)
    {
        if ((int)response.StatusCode is >= 300 and < 400)
        {
            ...
            if (location is null || !location.IsAbsoluteUri || !NeoOriginPolicy.SameOrigin(configuredUrl, location)) throw new NeoToolException("readiness_redirect", "The development server redirected readiness to an unexpected origin.");
...
internal sealed class NeoDevelopmentOrchestrator(INeoProcessFactory processFactory, INeoReadinessProbe readiness)
{
    internal async Task<int> RunAsync(NeoResolvedProject project, CancellationToken cancellationToken)
    {
        ...
        await using var frontend = processFactory.Start(new("frontend", project.DevCommand, project.FrontendRoot, project.Environment, project.SecretEnvironment, (line, error) => Log("frontend", line, error)));
        try
        {
            ...
            if (completed == cancellation) return 0;
        ...
        finally { await frontend.StopAsync(TimeSpan.FromSeconds(10)).ConfigureAwait(false); }
    }
}
