...
internal sealed unsafe class OwnedUtf8String : IDisposable
{
    ...
    internal NativeMethods.neo_webview_string_view_t View => new(new NativeMethods.neo_webview_string_view
    {
        data = _data,
...
internal sealed unsafe class CustomSchemeMarshaller : IDisposable
{
    ...
    private NativeMethods.neo_webview_custom_scheme_t* _schemes;
    ...
    internal CustomSchemeMarshaller(IReadOnlyList<NeoCustomScheme> schemes)
    {
        ...
        try
        {
            _schemes = (NativeMethods.neo_webview_custom_scheme_t*)NativeMemory.Alloc(
                checked((nuint)schemes.Count), (nuint)sizeof(NativeMethods.neo_webview_custom_scheme_t));
            NativeMemory.Clear(_schemes, checked((nuint)schemes.Count * (nuint)sizeof(NativeMethods.neo_webview_custom_scheme_t)));
            for (var index = 0; index < schemes.Count; index++)
            {
                var scheme = schemes[index];
                var name = AddString(scheme.Name);
                var origins = (NativeMethods.neo_webview_string_view_t*)null;
                if (scheme.AllowedOrigins.Count != 0)
                {
                    origins = (NativeMethods.neo_webview_string_view_t*)NativeMemory.Alloc(
                        checked((nuint)scheme.AllowedOrigins.Count), (nuint)sizeof(NativeMethods.neo_webview_string_view_t));
                ...
                _schemes[index] = new NativeMethods.neo_webview_custom_scheme_t(new NativeMethods.neo_webview_custom_scheme
                {
                    size = (uint)sizeof(NativeMethods.neo_webview_custom_scheme),
                    version = 1,
                    name = name,
                    flags = flags,
                    allowed_origin_count = (uint)scheme.AllowedOrigins.Count,
                    allowed_origins = origins,
                    resource_provider = (delegate* unmanaged[Cdecl]<void*, NativeMethods.neo_webview_resource_request_t*, NativeMethods.neo_webview_resource_response_t*, NativeMethods.neo_webview_result_t>)&ResourceProviderRegistration.Invoke,
    ...
    internal uint Stride => _schemes is null ? 0u : (uint)sizeof(NativeMethods.neo_webview_custom_scheme_t);
    internal NativeMethods.neo_webview_custom_scheme_t* Schemes => _schemes;
    ...
    private NativeMethods.neo_webview_string_view_t AddString(string value)
    {
...
{
    private readonly List<OwnedUtf8String> _strings = [];
    private NativeMethods.neo_webview_string_view_t* _views;
    ...
    internal Utf8StringArray(IReadOnlyList<string> values)
    {
        ...
        try
        {
            _views = (NativeMethods.neo_webview_string_view_t*)NativeMemory.Alloc(
                checked((nuint)values.Count), (nuint)sizeof(NativeMethods.neo_webview_string_view_t));
    ...
    internal NativeMethods.neo_webview_string_view_t* Views => _views;
...
internal sealed unsafe class ResourceProviderRegistration : IDisposable
{
    ...
    [UnmanagedCallersOnly(CallConvs = [typeof(CallConvCdecl)])]
    internal static NativeMethods.neo_webview_result_t Invoke(
        void* context,
        NativeMethods.neo_webview_resource_request_t* request,
        NativeMethods.neo_webview_resource_response_t* response)
    {
        try
        {
            if (context is null || request is null || response is null ||
                request->Value.size < sizeof(NativeMethods.neo_webview_resource_request) || request->Value.version != 1 ||
                (request->Value.body_length != 0 && request->Value.body == null) ||
                (uint)request->Value.resource_kind.Value > (uint)NativeMethods.neo_webview_resource_kind.NEO_WEBVIEW_RESOURCE_MANIFEST)
            {
            ...
            *response = new NativeMethods.neo_webview_resource_response_t(raw);
            return NativeMethods.neo_webview_result.NEO_WEBVIEW_OK;
        }
        catch
        {
            return NativeMethods.neo_webview_result.NEO_WEBVIEW_ERROR_NATIVE_FAILURE;
        }
    }
    ...
    private sealed class NativeResourceResponseLease : IDisposable
    {
        ...
        internal NativeMethods.neo_webview_resource_response CreateNative(void* context)
            => new()
            {
                size = (uint)sizeof(NativeMethods.neo_webview_resource_response),
                version = 1,
                status_code = (uint)_response.StatusCode,
                body_kind = _response.FilePath is not null
                    ? NativeMethods.neo_webview_resource_body_kind.NEO_WEBVIEW_RESOURCE_BODY_FILE
                    : _response.Bytes.IsEmpty
                        ? NativeMethods.neo_webview_resource_body_kind.NEO_WEBVIEW_RESOURCE_BODY_EMPTY
                        : NativeMethods.neo_webview_resource_body_kind.NEO_WEBVIEW_RESOURCE_BODY_BYTES,
