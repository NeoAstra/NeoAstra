...
/// <summary>Represents a NeoWebView-owned top-level native window.</summary>
public sealed unsafe class NeoWindow : IAsyncDisposable
{
    ...
    internal NeoWindow(NeoApplication application, SafeWindowHandle handle, NeoWindowOptions options)
    {
        ...
        Id = NativeMethods.neo_webview_window_get_id(NativeHandle);
    }
    ...
    public string Title
    {
        ...
        {
            ThrowIfDisposed();
            _title = Utf8String.Decode(NativeMethods.neo_webview_window_get_title(NativeHandle));
            return _title;
        }
        set
        {
            ...
            NativeError.ThrowIfFailed(NativeMethods.neo_webview_window_set_title(NativeHandle, utf8.View), default, "set window title");
            _title = value;
        }
    }
    ...
    {
        ThrowIfDisposed();
        NativeError.ThrowIfFailed(NativeMethods.neo_webview_window_show(NativeHandle), default, "show window");
        _isVisible = true;
    }
    ...
    {
        ThrowIfDisposed();
        NativeError.ThrowIfFailed(NativeMethods.neo_webview_window_hide(NativeHandle), default, "hide window");
        _isVisible = false;
    }
    ...
    {
        ThrowIfDisposed();
        NativeError.ThrowIfFailed(NativeMethods.neo_webview_window_activate(NativeHandle), default, "activate window");
    }
    ...
    {
        ThrowIfDisposed();
        NativeError.ThrowIfFailed(NativeMethods.neo_webview_window_close(NativeHandle), default, "close window");
    }
    ...
    public NeoNativeHandle GetNativeHandle(NeoNativeHandleKind kind)
    {
        ...
        var result = NativeMethods.neo_webview_window_get_native_handle(NativeHandle, (NativeMethods.neo_webview_native_handle_kind)kind, &native);
    ...
    internal NativeMethods.neo_webview_window_t NativeHandle
    {
        get
        {
            ThrowIfDisposed();
    ...
    {
        ThrowIfDisposed();
        var native = new NativeMethods.neo_webview_rect_t(default);
        var result = NativeMethods.neo_webview_window_get_bounds(NativeHandle, &native);
    ...
    {
        ThrowIfDisposed();
        var native = new NativeMethods.neo_webview_rect
        {
            x = value.X,
            y = value.Y,
            width = value.Width,
        ...
        NativeError.ThrowIfFailed(NativeMethods.neo_webview_window_set_bounds(NativeHandle, native), default, "set window bounds");
        _bounds = value;
    }
    ...
    private void DisposeCore(bool requestClose)
    {
        ...
        if (requestClose && Volatile.Read(ref _closed) == 0)
        {
            try
            {
                NativeMethods.neo_webview_window_close(new(_handle.DangerousGetHandle()));
            }
            catch
            {
