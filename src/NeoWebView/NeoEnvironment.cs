...
public sealed unsafe class NeoEnvironment : IAsyncDisposable
{
    ...
    public ValueTask<NeoProfile> CreateProfileAsync(NeoProfileOptions? options = null, CancellationToken cancellationToken = default)
    {
        ...
        var raw = new NativeMethods.neo_webview_profile_options
        {
            size = (uint)sizeof(NativeMethods.neo_webview_profile_options),
            version = 1,
            name = name.View,
            ephemeral = options.IsEphemeral ? 1u : 0u,
        };
        var nativeOptions = new NativeMethods.neo_webview_profile_options_t(raw);
        ...
        try
        {
            result = NativeMethods.neo_webview_environment_create_profile_async(
                NativeHandle,
                &nativeOptions,
                (delegate* unmanaged[Cdecl]<void*, NativeMethods.neo_webview_result_t, NativeMethods.neo_webview_profile_t, NativeMethods.neo_webview_error_t, void>)&ProfileCreated,
    ...
    private ValueTask<NeoWebView> CreateWebViewCoreAsync(NeoWebViewHost host, NeoWebViewOptions? options, nint popupRequest, CancellationToken cancellationToken)
    {
        ...
        if (host.Parent is { } nativeParent)
        {
            ...
            {
                NeoNativeHandleKind.Win32Hwnd => NativeMethods.neo_webview_native_parent_kind.NEO_WEBVIEW_NATIVE_PARENT_WIN32_HWND,
                NeoNativeHandleKind.CocoaNSView => NativeMethods.neo_webview_native_parent_kind.NEO_WEBVIEW_NATIVE_PARENT_COCOA_NSVIEW,
                NeoNativeHandleKind.GtkWidget => NativeMethods.neo_webview_native_parent_kind.NEO_WEBVIEW_NATIVE_PARENT_GTK_WIDGET,
                _ => NativeMethods.neo_webview_native_parent_kind.NEO_WEBVIEW_NATIVE_PARENT_NONE,
            };
            parent.handle = (void*)nativeParent.Value;
        }

        var raw = new NativeMethods.neo_webview_view_options
        {
            size = (uint)sizeof(NativeMethods.neo_webview_view_options),
            version = 1,
        ...
        try
        {
            result = NativeMethods.neo_webview_environment_create_view_async(
                NativeHandle,
                &nativeOptions,
                (delegate* unmanaged[Cdecl]<void*, NativeMethods.neo_webview_result_t, NativeMethods.neo_webview_view_t, NativeMethods.neo_webview_error_t, void>)&ViewCreated,
                (void*)operation.Context,
                &nativeOperation,
                &error);
        }
    ...
    public NeoCapabilityInfo GetCapability(NeoCapability capability)
    {
        ...
        var raw = new NativeMethods.neo_webview_capability_info
        {
            size = (uint)sizeof(NativeMethods.neo_webview_capability_info),
            version = 1,
        };
        var native = new NativeMethods.neo_webview_capability_info_t(raw);
        var result = NativeMethods.neo_webview_environment_get_capability(NativeHandle, (NativeMethods.neo_webview_capability)capability, &native);
    ...
    internal NativeMethods.neo_webview_environment_t NativeHandle
    {
        get
        {
            ThrowIfDisposed();
    ...
    {
        var raw = new NativeMethods.neo_webview_runtime_info
        {
            size = (uint)sizeof(NativeMethods.neo_webview_runtime_info),
            version = 1,
        };
        var native = new NativeMethods.neo_webview_runtime_info_t(raw);
        NativeMethods.neo_webview_error_t error = default;
        var result = NativeMethods.neo_webview_get_runtime_info(&native, &error);
    ...
    private static void ProfileCreated(void* context, NativeMethods.neo_webview_result_t result, NativeMethods.neo_webview_profile_t profile, NativeMethods.neo_webview_error_t error)
    {
        try
        {
    ...
    [UnmanagedCallersOnly(CallConvs = [typeof(CallConvCdecl)])]
    private static void ViewCreated(void* context, NativeMethods.neo_webview_result_t result, NativeMethods.neo_webview_view_t view, NativeMethods.neo_webview_error_t error)
    {
        try
        {
            var operation = NativeOperation.Get<NeoWebView>(context);
            if (operation is null)
            {
            ...
            if (NativeError.Code(result) == NeoErrorCode.Success && view.Handle != 0)
            {
                ...
                operation.Complete(new NeoWebView(creation.Environment, new SafeViewHandle(view.Handle), creation.Host, creation.Options));
            }
            else
            {
                if (view.Handle != 0) new SafeViewHandle(view.Handle).Dispose();
        ...
        {
            NativeOperation.Get<NeoWebView>(context)?.Fail(ex);
        }
    }

    private sealed record ProfileCreation(NeoEnvironment Environment, bool IsEphemeral);
    private sealed record ViewCreation(NeoEnvironment Environment, NeoWebViewHost Host, NeoWebViewOptions Options);
}
