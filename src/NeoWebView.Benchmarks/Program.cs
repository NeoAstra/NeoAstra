...
internal static class Program
{
    ...
    private static int Main(string[] args)
    {
        ...
        if (!BenchmarkOptions.TryParse(args, out var options))
        {
            Console.Error.WriteLine(
                "Usage: dotnet run --project src/NeoWebView.Benchmarks -c Release -- --run [--quick] " +
        ...
        try
        {
            return NeoApplication.Run(
                ...
                {
                    ApplicationName = "NeoWebView Benchmarks",
    ...
    private sealed class BenchmarkRunner(NeoApplication application, BenchmarkOptions options)
    {
        ...
        internal async ValueTask RunAsync()
        {
            ...
            Console.WriteLine(
                "NOTE Measurements include NeoWebView, native backend, browser engine, OS scheduling, and machine noise. " +
                "Engine startup and process behavior are not controlled entirely by NeoWebView.");
            ...
            var window = CreateHiddenWindow("NeoWebView benchmark host");
            try
            {
            ...
            NeoWebView.NeoWebView view)
        {
            ...
            Reporter.Measurement(
                ...
                "us/op",
                callCount,
                "NeoWebView.ZoomFactor native getter; excludes cached managed properties");
        ...
        private async ValueTask MeasureJavaScriptRoundTripsAsync(NeoWebView.NeoWebView view)
        {
        ...
        private async ValueTask MeasureMessagingAsync(NeoWebView.NeoWebView view, BridgeMode bridgeMode)
        {
        ...
        private async ValueTask NavigateAndWaitAsync(NeoWebView.NeoWebView view, Uri uri)
        {
    ...
    private static NeoWebViewOptions CreateViewOptions(BridgeMode bridgeMode)
    {
        var options = new NeoWebViewOptions { MaximumMessageSize = 1024 * 1024 };
