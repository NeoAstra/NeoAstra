...
{
    private static readonly HashSet<string> InstallerExecutables = new(StringComparer.OrdinalIgnoreCase)
    {
        "bunx",
        "npx",
        "pnpx"
    };
    private static readonly HashSet<string> InstallerVerbs = new(StringComparer.OrdinalIgnoreCase)
    {
        "add",
        "ci",
        "create",
...
internal sealed record NeoResolvedProject(
    ...
    NeoCommand PackageManagerCommand,
    string? Lockfile,
    ...
    NeoBundleConfiguration? Bundle)
{
    internal string ToInspectJson(bool redactSecrets)
    {
        ...
        foreach (var pair in Environment)
            environment[pair.Key] = redactSecrets && SecretEnvironment.Contains(pair.Key) ? "[REDACTED]" : pair.Value;
        using var stream = new MemoryStream();
        using (var writer = new Utf8JsonWriter(stream, new JsonWriterOptions { Indented = true }))
        {
            writer.WriteStartObject();
            writer.WriteNumber("version", 1);
            writer.WriteString("configurationPath", ConfigurationPath);
            writer.WriteString("projectDirectory", ProjectDirectory);
            ...
            WriteArray(writer, "contractCommand", ContractCommand.Arguments);
            writer.WriteString("devUrl", DevUrl.AbsoluteUri);
            WriteArray(writer, "buildCommand", BuildCommand.Arguments);
            writer.WriteString("dist", DistDirectory);
            writer.WriteString("spaFallback", SpaFallback);
            writer.WriteString("packageManager", PackageManager);
            WriteArray(writer, "packageManagerCommand", PackageManagerCommand.Arguments);
            WriteOptional(writer, "lockfile", Lockfile);
            WriteOptional(writer, "generatedContract", GeneratedContract);
            writer.WriteBoolean("allowRemoteDevServer", AllowRemoteDevServer);
            ...
            writer.WriteString("origin", ProductionOrigin.AbsoluteUri.TrimEnd('/'));
            writer.WriteBoolean("cacheHashedAssets", CacheHashedAssets);
            writer.WriteString("csp", ContentSecurityPolicy);
            writer.WriteString("referrerPolicy", ReferrerPolicy);
            writer.WriteBoolean("includeSourceMaps", IncludeSourceMaps);
            writer.WriteNumber("maximumFiles", MaximumFiles);
            writer.WriteNumber("maximumFileBytes", MaximumFileBytes);
            writer.WriteNumber("maximumTotalBytes", MaximumTotalBytes);
            WriteArray(writer, "spaRoutePrefixes", SpaRoutePrefixes);
            WriteArray(writer, "excludedPrefixes", ExcludedPrefixes);
            writer.WriteEndObject();
            WriteArray(writer, "capabilities", Capabilities);
            Bundle?.WriteInspect(writer);
            writer.WriteEndObject();
        }
...
internal static class NeoProjectConfiguration
{
    ...
    internal static NeoResolvedProject Load(string path, IReadOnlyDictionary<string, string>? overrides = null)
    {
        ...
        if (!info.Exists)
            return LoadConventions(fullPath, overrides);
        if (info.Length is < 1 or > MaximumConfigurationBytes)
            throw new NeoToolException("configuration_file", "neoastra.json must be between 1 byte and 1 MiB.");
        var bytes = File.ReadAllBytes(fullPath);
        if (bytes.Length is < 1 or > MaximumConfigurationBytes)
            throw new NeoToolException("configuration_file", "neoastra.json must be between 1 byte and 1 MiB.");
    ...
    private static NeoResolvedProject LoadConventions(string configurationPath, IReadOnlyDictionary<string, string>? overrides)
    {
        ...
        var devArguments = packageManager == "npm" ? new[]
        {
            executable,
            "run",
            "dev",
        ...
        var devUrl = NeoOriginPolicy.ValidateDevelopmentUrl(Override(overrides, "NeoAstraDevUrl") ?? "http://127.0.0.1:5173", allowRemote: false);
        return new NeoResolvedProject(
            ...
            frontendRoot,
            new NeoCommand(devArguments),
            new NeoCommand(["dotnet", "watch", "run"]),
            new NeoCommand(["dotnet", "build", "--no-restore"]),
            devUrl,
            new NeoCommand(buildArguments),
            Path.Combine(frontendRoot, "dist"),
            "index.html",
            packageManager,
    ...
    internal static NeoResolvedProject ValidateGenerated(string path, string json)
    {
        ...
        if (bytes.Length is < 1 or > MaximumConfigurationBytes)
            throw new NeoToolException("configuration_file", "Generated neoastra.json must be between 1 byte and 1 MiB.");
    ...
    private static NeoResolvedProject Parse(byte[] bytes, string fullPath, IReadOnlyDictionary<string, string>? overrides)
    {
        try
        {
            ...
            if (String(root, "$schema") != "neoastra-project-v1.schema.json" || Integer(root, "version") != 1)
                throw Error("version", "Only neoastra project schema version 1 is supported.");
            var app = RequiredObject(root, "app");
            Exact(app, "identifier", "displayName");
            var identifier = String(app, "identifier", 255);
            if (!IdentifierPattern.IsMatch(identifier))
                throw Error("app.identifier", "The application identifier is invalid.");
            var displayName = String(app, "displayName", 128);
            var frontend = RequiredObject(root, "frontend");
            Exact(frontend, "root", "devCommand", "backendCommand", "contractCommand", "devUrl", "buildCommand", "dist", "spaFallback", "packageManager", "packageManagerCommand", "lockfile", "generatedContract", "allowRemoteDevServer", "environment", "secretEnvironment");
            ...
            if (secrets.Any(name => !environment.ContainsKey(name)))
                throw Error("frontend.secretEnvironment", "Every secret environment name must be declared in frontend.environment.");
            var packageManager = OptionalString(frontend, "packageManager") ?? "none";
            if (packageManager is not ("npm" or "pnpm" or "yarn" or "bun" or "none"))
                throw Error("frontend.packageManager", "Unknown package manager.");
            var configuredPackageManagerCommand = OptionalCommand(frontend, "packageManagerCommand");
            if (packageManager == "none" && configuredPackageManagerCommand is not null)
                throw Error("frontend.packageManagerCommand", "A package-manager command cannot be configured when packageManager is none.");
            var packageManagerCommand = configuredPackageManagerCommand ?? NeoPackageManagerCommandResolver.Discover(packageManager, frontendRoot);
            ...
            var csp = String(assets, "csp", 16384);
            if (csp.Contains("unsafe-eval", StringComparison.OrdinalIgnoreCase) || csp.Contains("*", StringComparison.Ordinal))
                throw Error("assets.csp", "Production CSP must not contain unsafe-eval or wildcard sources.");
            ...
            return new(
                ...
                OptionalCommand(frontend, "backendCommand") ?? new NeoCommand(["dotnet", "watch", "run"]),
                OptionalCommand(frontend, "contractCommand") ?? new NeoCommand(["dotnet", "build", "--no-restore"]),
                devUrl,
                Command(frontend, "buildCommand"),
                dist,
                spaFallback,
                packageManager,
                packageManagerCommand,
                ...
                secrets,
                origin,
                Boolean(assets, "cacheHashedAssets", true),
                csp,
                OptionalString(assets, "referrerPolicy", 128) ?? "no-referrer",
                Boolean(assets, "includeSourceMaps", false),
                BoundedInteger(assets, "maximumFiles", 10_000, 1, 50_000),
                BoundedLong(assets, "maximumFileBytes", 64L * 1024 * 1024, 1, 1024L * 1024 * 1024),
        ...
        {
            throw new NeoToolException("invalid_json", $"neoastra.json is invalid JSON at byte {exception.BytePositionInLine}.");
        }
    ...
    {
        if (!ProductionOriginPattern.IsMatch(value)
            || !Uri.TryCreate(value, UriKind.Absolute, out var uri))
        {
            throw Error("assets.origin", "Production origin must be a canonical lowercase authority-based non-standard custom-scheme origin without path, query, fragment, or credentials.");
        }

        var hasInvalidComponent = uri.UserInfo.Length != 0
            || uri.Query.Length != 0
            || uri.Fragment.Length != 0
            || uri.AbsolutePath != "/"
            || string.IsNullOrEmpty(uri.Host);
        var hasReservedScheme = uri.Scheme is "about" or "blob" or "data" or "file" or "ftp"
            or "http" or "https" or "javascript" or "ws" or "wss";
        var isCanonical = value.Equals(uri.GetLeftPart(UriPartial.Authority), StringComparison.Ordinal);
        if (hasInvalidComponent || hasReservedScheme || !isCanonical)
        {
            throw Error("assets.origin", "Production origin must be a canonical lowercase authority-based non-standard custom-scheme origin without path, query, fragment, or credentials.");
        }
    ...
    private static string ResolvePath(string root, string value, string field)
    {
        if (value.IndexOf('\0') >= 0)
            throw Error(field, "A path contains NUL.");
    ...
    {
        if (!value.TryGetProperty(name, out var command))
            return null;
        if (command.ValueKind != JsonValueKind.Array || command.GetArrayLength() is < 1 or > 256)
            throw Error(name, "A command must be an array of 1 through 256 arguments.");
    ...
    private static Dictionary<string, string> ReadEnvironment(JsonElement frontend)
    {
        ...
        if (!frontend.TryGetProperty("environment", out var value))
            return result;
        if (value.ValueKind != JsonValueKind.Object)
            throw Error("frontend.environment", "Environment must be an object.");
        foreach (var property in value.EnumerateObject())
        {
            if (result.Count == 128 || property.Name.Length > 1024 || !Regex.IsMatch(property.Name, "^[A-Za-z_][A-Za-z0-9_]*$", RegexOptions.CultureInvariant) || property.Value.ValueKind != JsonValueKind.String || property.Value.GetString()!.Length > 32768)
                throw Error("frontend.environment", "Environment additions are invalid or exceed their bounds.");
    ...
    {
        if (!value.TryGetProperty(name, out var routes))
            return defaults ?? [];
        var result = ReadStrings(value, name, 128, false).ToList();
        if (result.Any(route => !ValidRoute(route)))
            throw Error(name, "Route prefixes must be normalized absolute URL paths.");
        if (defaults is not null)
            foreach (var required in defaults)
                if (!result.Contains(required, StringComparer.Ordinal))
                    result.Add(required);
        if (result.Count > 128)
            throw Error(name, "Route prefixes exceed their bound after mandatory exclusions are applied.");
        return result;
    }
    ...
    private static bool ValidRoute(string route)
    {
        ...
        return route == "/"
            || route[1..].Split('/').All(static segment => segment is not ("" or "." or ".."));
    }
    private static IReadOnlyList<string> ReadStrings(JsonElement value, string name, int maximum, bool paths)
    {
        ...
        {
            if (item.ValueKind != JsonValueKind.String || item.GetString() is not { Length: > 0 and <= 1024 } text || !unique.Add(text) || (paths && text.IndexOf('\0') >= 0))
    ...
    private static string AssetPath(string value, string field)
    {
        ...
        if (normalized.StartsWith('/') || normalized.Contains(':') || normalized.Contains('\0') || normalized.Split('/').Any(segment => segment is "" or "." or ".."))
            throw Error(field, "Asset path must be a normalized relative path.");
        return normalized;
    }
    ...
    {
        if (value.ValueKind != JsonValueKind.Object)
            throw Error("object", "A configuration section must be an object.");
        var allowed = names.ToHashSet(StringComparer.Ordinal);
        foreach (var property in value.EnumerateObject())
            if (!allowed.Contains(property.Name))
                throw Error(property.Name, "Unknown configuration field.");
    }
    ...
    private static void ValidateComplexity(JsonElement root)
    {
        ...
        {
            if (++nodes > 50_000)
                throw Error("complexity", "Configuration exceeds its structural complexity bound.");
            if (element.ValueKind == JsonValueKind.Object)
            {
                ...
                foreach (var property in element.EnumerateObject())
                {
                    if (!names.Add(property.Name))
                        throw Error(property.Name, "Duplicate JSON property.");
