...
internal static class NeoProjectConfiguration
{
    ...
    internal static NeoResolvedProject Load(string path, IReadOnlyDictionary<string, string>? overrides = null)
    {
        ...
        if (!info.Exists) return LoadConventions(fullPath, overrides);
        if (info.Length is < 1 or > MaximumConfigurationBytes) throw new NeoToolException("configuration_file", "neoastra.json must be between 1 byte and 1 MiB.");
    ...
    private static NeoResolvedProject LoadConventions(string configurationPath, IReadOnlyDictionary<string, string>? overrides)
    {
        ...
        var devArguments = packageManager == "npm" ? new[] { executable, "run", "dev", "--", "--host", "127.0.0.1" } : new[] { executable, "run", "dev", "--host", "127.0.0.1" };
        var buildArguments = new[] { executable, "run", "build" };
