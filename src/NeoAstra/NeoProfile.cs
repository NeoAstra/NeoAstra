...
public sealed unsafe class NeoProfile : IAsyncDisposable
{
    ...
    public ValueTask<IReadOnlyList<NeoCookie>> GetCookiesAsync(Uri uri, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            result = NativeMethods.neoastra_profile_get_cookies_async(
                NativeHandle,
                nativeUri.View,
                (delegate* unmanaged[Cdecl]<void*, NativeMethods.neoastra_result_t, NativeMethods.neoastra_buffer_t, NativeMethods.neoastra_error_t, void>)&CookiesCompleted,
    ...
    public ValueTask ClearDataAsync(NeoBrowsingDataKinds kinds, NeoTimeRange? timeRange = null, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            result = NativeMethods.neoastra_profile_clear_data_async(
                NativeHandle,
                (ulong)kinds,
                start,
                end,
                (delegate* unmanaged[Cdecl]<void*, NativeMethods.neoastra_result_t, NativeMethods.neoastra_error_t, void>)&Completion,
                (void*)operation.Context,
                &nativeOperation,
                &error);
        }
    ...
    internal NativeMethods.neoastra_profile_t NativeHandle
    {
        get
        {
            ThrowIfDisposed();
    ...
    private ValueTask StartCookieOperation(NeoCookie cookie, bool delete, CancellationToken cancellationToken)
    {
        ...
        var raw = new NativeMethods.neoastra_cookie
        {
            size = (uint)sizeof(NativeMethods.neoastra_cookie),
            version = 1,
            name = name.View,
            value = value.View,
            domain = domain.View,
        ...
        var nativeCookie = new NativeMethods.neoastra_cookie_t(raw);
        ...
        {
            result = delete
                ? NativeMethods.neoastra_profile_delete_cookie_async(NativeHandle, &nativeCookie, (delegate* unmanaged[Cdecl]<void*, NativeMethods.neoastra_result_t, NativeMethods.neoastra_error_t, void>)&Completion, (void*)operation.Context, &nativeOperation, &error)
                : NativeMethods.neoastra_profile_set_cookie_async(NativeHandle, &nativeCookie, (delegate* unmanaged[Cdecl]<void*, NativeMethods.neoastra_result_t, NativeMethods.neoastra_error_t, void>)&Completion, (void*)operation.Context, &nativeOperation, &error);
        }
        catch (Exception ex)
        {
            operation.FailStart(ex);
    ...
    private static void CompleteStart<T>(NativeOperation<T> operation, NativeMethods.neoastra_operation_t nativeOperation, NativeMethods.neoastra_result_t result, NativeMethods.neoastra_error_t error, string name, CancellationToken cancellationToken)
    {
    ...
    private static void Completion(void* context, NativeMethods.neoastra_result_t result, NativeMethods.neoastra_error_t error)
    {
        try
        {
    ...
    [UnmanagedCallersOnly(CallConvs = [typeof(CallConvCdecl)])]
    private static void CookiesCompleted(void* context, NativeMethods.neoastra_result_t result, NativeMethods.neoastra_buffer_t buffer, NativeMethods.neoastra_error_t error)
    {
    ...
    private static IReadOnlyList<NeoCookie> ParseCookies(NativeMethods.neoastra_buffer_t buffer)
    {
        if (buffer.Handle == 0)
        {
        ...
        var length = NativeMethods.neoastra_buffer_get_length(buffer);
        if (length == 0)
        {
            return Array.Empty<NeoCookie>();
        }
        ...
        var data = NativeMethods.neoastra_buffer_get_data(buffer);
        if (data is null)
        {
