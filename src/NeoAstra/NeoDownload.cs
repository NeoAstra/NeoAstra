...
public sealed class NeoDownload : IDisposable
{
    ...
    {
        NativeMethods.neoastra_download_retain(new(handle));
    ...
    public unsafe void Refresh()
    {
        ...
        var raw = new NativeMethods.neoastra_download_info
        {
            size = (uint)sizeof(NativeMethods.neoastra_download_info),
            version = 1,
        };
        var info = new NativeMethods.neoastra_download_info_t(raw);
        NativeError.ThrowIfFailed(NativeMethods.neoastra_download_get_info(new(_handle.DangerousGetHandle()), &info), default, "get download information");
        raw = info.Value;
        Id = raw.id;
    ...
    {
        ObjectDisposedException.ThrowIf(Volatile.Read(ref _disposed) != 0, this);
        NativeError.ThrowIfFailed(NativeMethods.neoastra_download_cancel(new(_handle.DangerousGetHandle())), default, "cancel download");
    }
    ...
    {
        ObjectDisposedException.ThrowIf(Volatile.Read(ref _disposed) != 0, this);
        NativeError.ThrowIfFailed(NativeMethods.neoastra_download_pause(new(_handle.DangerousGetHandle())), default, "pause download");
    }
    ...
    {
        ObjectDisposedException.ThrowIf(Volatile.Read(ref _disposed) != 0, this);
        NativeError.ThrowIfFailed(NativeMethods.neoastra_download_resume(new(_handle.DangerousGetHandle())), default, "resume download");
    }
