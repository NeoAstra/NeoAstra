...
export const SUPPORTED_FEATURES = Object.freeze(["invoke", "cancel", "events"]);
...
export class NeoAstraClientError extends Error {
    ...
    correlationId;
    retryable;
    constructor(code, message, retryable = false, correlationId) {
        super(message);
...
export function frameByteLength(value) {
    let json;
    try {
...
export function assertApplicationFrame(value, maximumFrameBytes, maximumJsonDepth = DEFAULT_MAXIMUM_JSON_DEPTH) {
    ...
    while (pending.length !== 0) {
        ...
        if (current.depth > maximumJsonDepth)
            throw new NeoAstraClientError("invalid_frame", "The transport frame exceeds the negotiated JSON depth.");
