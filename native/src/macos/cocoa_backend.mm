...
namespace {
struct cocoa_app { id previous_delegate; id delegate; neoastra_decision_t* termination_decision{}; };
struct cocoa_window;
...
@end
@interface NeoApplicationDelegate : NSObject<NSApplicationDelegate>
@property(nonatomic,assign) neoastra_app_t* nativeApplication;
@property(nonatomic,strong) id<NSApplicationDelegate> previousDelegate;
@end
...
@implementation NeoApplicationDelegate
...
- (BOOL)applicationShouldHandleReopen:(NSApplication*)sender hasVisibleWindows:(BOOL)visible { auto* app=self.nativeApplication;if(app)neo_emit_app(app,NEOASTRA_EVENT_APPLICATION_ACTIVATED);id<NSApplicationDelegate> previous=self.previousDelegate;return previous&&[previous respondsToSelector:@selector(applicationShouldHandleReopen:hasVisibleWindows:)]?[previous applicationShouldHandleReopen:sender hasVisibleWindows:visible]:YES; }
- (NSApplicationTerminateReply)applicationShouldTerminate:(NSApplication*)sender { id<NSApplicationDelegate> previous=self.previousDelegate;const auto previousReply=previous&&[previous respondsToSelector:@selector(applicationShouldTerminate:)]?[previous applicationShouldTerminate:sender]:NSTerminateNow;if(previousReply!=NSTerminateNow)return previousReply;auto* app=self.nativeApplication;if(!app)return NSTerminateNow;auto* state=static_cast<cocoa_app*>(app->platform);if(!state)return NSTerminateNow;if(state->termination_decision)return NSTerminateLater;auto* decision=new(std::nothrow) neoastra_decision_t;if(!decision){neo_emit_app(app,NEOASTRA_EVENT_APPLICATION_SESSION_END,0,nullptr,nullptr,2);return NSTerminateNow;}decision->kind=NEOASTRA_DECISION_APPLICATION_QUIT;decision->default_action=NEOASTRA_DECISION_CANCEL;decision->deadline=std::chrono::steady_clock::now()+std::chrono::seconds(30);decision->attach_app(app);decision->completion=[](void* context,const neoastra_decision_response_t* response) noexcept {@autoreleasepool{auto* nativeApp=static_cast<neoastra_app_t*>(context);auto* nativeState=nativeApp?static_cast<cocoa_app*>(nativeApp->platform):nullptr;if(nativeState&&nativeState->termination_decision){auto* retained=nativeState->termination_decision;nativeState->termination_decision=nullptr;retained->release();}[NSApp replyToApplicationShouldTerminate:response&&(response->action==NEOASTRA_DECISION_ALLOW||response->action==NEOASTRA_DECISION_DEFAULT)];}};decision->completion_context=app;decision->retain();state->termination_decision=decision;neo_emit_app(app,NEOASTRA_EVENT_APPLICATION_SESSION_END,0,nullptr,nullptr,0,1,decision);neo_finish_decision_event(app,decision);decision->release();return NSTerminateLater; }
- (void)application:(NSApplication*)sender openFiles:(NSArray<NSString*>*)filenames { auto* app=self.nativeApplication;if(app)for(NSString* filename in filenames){auto value=utf8(filename);if(value.size()<=4096)neo_emit_app(app,NEOASTRA_EVENT_APPLICATION_OPEN_FILE,0,&value);}id<NSApplicationDelegate> previous=self.previousDelegate;if(previous&&[previous respondsToSelector:@selector(application:openFiles:)])[previous application:sender openFiles:filenames];else[sender replyToOpenOrPrint:NSApplicationDelegateReplySuccess]; }
- (void)application:(NSApplication*)application openURLs:(NSArray<NSURL*>*)urls { auto* app=self.nativeApplication;if(app)for(NSURL* url in urls){auto value=utf8(url.absoluteString);if(value.size()<=4096)neo_emit_app(app,NEOASTRA_EVENT_APPLICATION_OPEN_URL,0,nullptr,&value);}id<NSApplicationDelegate> previous=self.previousDelegate;if(previous&&[previous respondsToSelector:@selector(application:openURLs:)])[previous application:application openURLs:urls]; }
@end
...
void neo_platform_shutdown(neoastra_app_t* app) noexcept {auto* state=static_cast<cocoa_app*>(app->platform);if(state){if(state->termination_decision){auto* decision=state->termination_decision;state->termination_decision=nullptr;decision->completion=nullptr;decision->completion_context=nullptr;decision->abandon();decision->release();}((NeoApplicationDelegate*)state->delegate).nativeApplication=nullptr;if(NSApp.delegate==state->delegate)NSApp.delegate=state->previous_delegate;delete state;}app->platform=nullptr;}
...
neoastra_result_t neo_platform_window_force_close(neoastra_window_t* window) noexcept {auto* state=static_cast<cocoa_window*>(window->platform);if(!state||!state->window)return NEOASTRA_ERROR_DISPOSED;[state->window close];return NEOASTRA_OK;}
...
neoastra_result_t neo_platform_window_set_attribute(neoastra_window_t* window,neoastra_window_attribute_t attribute,bool enabled) noexcept {@autoreleasepool{auto* state=static_cast<cocoa_window*>(window->platform);if(!state||!state->window)return NEOASTRA_ERROR_DISPOSED;switch(attribute){case NEOASTRA_WINDOW_RESIZABLE:{auto style=state->window.styleMask;if(enabled)style|=NSWindowStyleMaskResizable;else style&=~NSWindowStyleMaskResizable;state->window.styleMask=style;break;}case NEOASTRA_WINDOW_DECORATED:{auto style=state->window.styleMask;if(enabled)style|=NSWindowStyleMaskTitled;else style&=~NSWindowStyleMaskTitled;state->window.styleMask=style;break;}case NEOASTRA_WINDOW_ALWAYS_ON_TOP:state->window.level=enabled?NSFloatingWindowLevel:NSNormalWindowLevel;break;case NEOASTRA_WINDOW_SHOW_IN_TASKBAR:return NEOASTRA_ERROR_NOT_SUPPORTED;default:return NEOASTRA_ERROR_INVALID_ARGUMENT;}return NEOASTRA_OK;}}
neoastra_result_t neo_platform_window_begin_drag(neoastra_window_t* window) noexcept {@autoreleasepool{auto* state=static_cast<cocoa_window*>(window->platform);if(!state||!state->window)return NEOASTRA_ERROR_DISPOSED;NSEvent* event=NSApp.currentEvent;if(!event)return NEOASTRA_ERROR_INVALID_STATE;[state->window performWindowDragWithEvent:event];return NEOASTRA_OK;}}
neoastra_result_t neo_platform_window_begin_resize(neoastra_window_t*,neoastra_window_resize_edge_t) noexcept {return NEOASTRA_ERROR_NOT_SUPPORTED;}
