...
struct gtk_view { GtkWidget* widget{}; WebKitUserContentManager* content{}; GtkDropTarget* drop_target{}; gulong load_changed{}; gulong load_failed{}; gulong title_changed{}; gulong uri_changed{}; gulong message_received{}; gulong process_terminated{}; gulong drop_received{}; uint64_t next_script{1}; std::unordered_map<std::string,WebKitUserScript*> scripts; };
...
gboolean window_close_requested(GtkWindow*,void* data){auto* window=static_cast<neoastra_window_t*>(data);if(window->force_closing){window->force_closing=false;auto* state=static_cast<gtk_window*>(window->platform);if(state)state->widget=nullptr;neo_window_closed(window);return FALSE;}neo_window_request_close(window,NEOASTRA_WINDOW_CLOSE_USER,true);return TRUE;}
void window_size_changed(GObject* object,GParamSpec*,void* data){auto* window=static_cast<neoastra_window_t*>(data);const auto width=gtk_widget_get_width(GTK_WIDGET(object)),height=gtk_widget_get_height(GTK_WIDGET(object));if(width<=0||height<=0)return;bool changed{};{std::lock_guard lock(window->state_mutex);changed=window->bounds.width!=width||window->bounds.height!=height;window->bounds.width=width;window->bounds.height=height;}if(changed)neo_emit_app(window->app,NEOASTRA_EVENT_WINDOW_RESIZED,window->id);}
...
neoastra_result_t neo_platform_window_force_close(neoastra_window_t* window) noexcept {auto* state=static_cast<gtk_window*>(window->platform);if(!state||!state->widget)return NEOASTRA_ERROR_DISPOSED;auto* widget=state->widget;state->widget=nullptr;gtk_window_destroy(GTK_WINDOW(widget));neo_window_closed(window);return NEOASTRA_OK;}
...
neoastra_result_t neo_platform_window_begin_drag(neoastra_window_t* window) noexcept {auto* state=static_cast<gtk_window*>(window->platform);return !state||!state->widget?NEOASTRA_ERROR_DISPOSED:NEOASTRA_ERROR_NOT_SUPPORTED;}
neoastra_result_t neo_platform_window_begin_resize(neoastra_window_t* window,neoastra_window_resize_edge_t) noexcept {auto* state=static_cast<gtk_window*>(window->platform);return !state||!state->widget?NEOASTRA_ERROR_DISPOSED:NEOASTRA_ERROR_NOT_SUPPORTED;}
