...
struct gtk_view { GtkWidget* widget{}; WebKitUserContentManager* content{}; gulong load_changed{}; gulong load_failed{}; gulong title_changed{}; gulong uri_changed{}; gulong message_received{}; gulong process_terminated{}; uint64_t next_script{1}; std::unordered_map<std::string,WebKitUserScript*> scripts; };
...
gboolean window_delete(GtkWidget*, GdkEvent*, void* data) { auto* window=static_cast<neoastra_window_t*>(data); neo_emit_app(window->app,NEOASTRA_EVENT_WINDOW_CLOSE_REQUESTED,window->id); return FALSE; }
...
neoastra_result_t neo_platform_window_close(neoastra_window_t* window) noexcept {auto* state=static_cast<gtk_window*>(window->platform);if(!state||!state->widget)return NEOASTRA_ERROR_DISPOSED;gtk_widget_destroy(state->widget);return NEOASTRA_OK;}
...
neoastra_result_t neo_platform_window_get_handle(neoastra_window_t* window,neoastra_native_handle_kind_t kind,neoastra_native_handle_t* handle) noexcept {if(kind!=NEOASTRA_NATIVE_HANDLE_GTK_WIDGET)return NEOASTRA_ERROR_NOT_SUPPORTED;auto* state=static_cast<gtk_window*>(window->platform);if(!state||!state->widget)return NEOASTRA_ERROR_DISPOSED;handle->kind=kind;handle->value=state->widget;return NEOASTRA_OK;}
