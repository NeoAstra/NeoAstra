...
gboolean window_configured(GtkWidget*,GdkEventConfigure* event,void* data){auto* window=static_cast<neoastra_window_t*>(data);bool changed{};{std::lock_guard lock(window->state_mutex);changed=window->bounds.x!=event->x||window->bounds.y!=event->y;window->bounds.x=event->x;window->bounds.y=event->y;}if(changed)neo_emit_app(window->app,NEOASTRA_EVENT_WINDOW_MOVED,window->id);return FALSE;}
gboolean window_focus(GtkWidget*,GdkEventFocus* event,void* data){auto* window=static_cast<neoastra_window_t*>(data);neo_emit_app(window->app,NEOASTRA_EVENT_WINDOW_FOCUS_CHANGED,window->id,nullptr,nullptr,event->in?1u:0u);return FALSE;}
...
void view_destroyed(GtkWidget* widget,void* data){auto* view=static_cast<neoastra_view_t*>(data);auto* state=view?static_cast<gtk_view*>(view->platform):nullptr;if(state&&state->widget==widget)state->widget=nullptr;}
...
neoastra_result_t neo_platform_window_set_attribute(neoastra_window_t* window,neoastra_window_attribute_t attribute,bool enabled) noexcept {auto* state=static_cast<gtk_window*>(window->platform);if(!state||!state->widget)return NEOASTRA_ERROR_DISPOSED;auto* value=GTK_WINDOW(state->widget);switch(attribute){case NEOASTRA_WINDOW_RESIZABLE:gtk_window_set_resizable(value,enabled);break;case NEOASTRA_WINDOW_DECORATED:gtk_window_set_decorated(value,enabled);break;case NEOASTRA_WINDOW_ALWAYS_ON_TOP:gtk_window_set_keep_above(value,enabled);break;case NEOASTRA_WINDOW_SHOW_IN_TASKBAR:gtk_window_set_skip_taskbar_hint(value,!enabled);break;default:return NEOASTRA_ERROR_INVALID_ARGUMENT;}return NEOASTRA_OK;}
static bool current_pointer_event(guint& button,gint& x,gint& y,guint32& time) noexcept {auto* event=gtk_get_current_event();if(!event)return false;gdouble root_x{},root_y{};const auto valid=gdk_event_get_button(event,&button)&&gdk_event_get_root_coords(event,&root_x,&root_y);time=gdk_event_get_time(event);gdk_event_free(event);x=static_cast<gint>(root_x);y=static_cast<gint>(root_y);return valid;}
neoastra_result_t neo_platform_window_begin_drag(neoastra_window_t* window) noexcept {auto* state=static_cast<gtk_window*>(window->platform);if(!state||!state->widget)return NEOASTRA_ERROR_DISPOSED;guint button{};gint x{},y{};guint32 time{};if(!current_pointer_event(button,x,y,time))return NEOASTRA_ERROR_INVALID_STATE;gtk_window_begin_move_drag(GTK_WINDOW(state->widget),static_cast<gint>(button),x,y,time);return NEOASTRA_OK;}
neoastra_result_t neo_platform_window_begin_resize(neoastra_window_t* window,neoastra_window_resize_edge_t edge) noexcept {auto* state=static_cast<gtk_window*>(window->platform);if(!state||!state->widget)return NEOASTRA_ERROR_DISPOSED;static constexpr GdkWindowEdge edges[]={GDK_WINDOW_EDGE_WEST,GDK_WINDOW_EDGE_NORTH,GDK_WINDOW_EDGE_EAST,GDK_WINDOW_EDGE_SOUTH,GDK_WINDOW_EDGE_NORTH_WEST,GDK_WINDOW_EDGE_NORTH_EAST,GDK_WINDOW_EDGE_SOUTH_WEST,GDK_WINDOW_EDGE_SOUTH_EAST};guint button{};gint x{},y{};guint32 time{};if(!current_pointer_event(button,x,y,time))return NEOASTRA_ERROR_INVALID_STATE;gtk_window_begin_resize_drag(GTK_WINDOW(state->widget),edges[edge],static_cast<gint>(button),x,y,time);return NEOASTRA_OK;}
