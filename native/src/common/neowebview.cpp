...
bool neo_emit_bridge_message(neo_webview_view_t* view, const std::string& message, const std::string& uri, bool main_frame) noexcept {
    ...
    if (!neo_bridge_access_allowed(view, uri)) {
        neo_log(view->environment->app, NEO_WEBVIEW_LOG_WARNING, "bridge", "Blocked a web message by the configured bridge policy");
        return false;
    }
...
neo_webview_profile::~neo_webview_profile() { destroy_ui_once(); environment->release(); }
void neo_webview_profile::destroy_ui() noexcept { neo_platform_profile_destroy(this); }
neo_webview_window::~neo_webview_window() {
    destroy_ui_once();
    if (owner) owner->release();
}
void neo_webview_window::destroy_ui() noexcept { neo_platform_window_destroy(this); }
...
void neo_webview_view::destroy_ui() noexcept {
    for (;;) {
...
bool valid_native_parent(const neo_webview_native_parent_t& parent) noexcept {
    const auto is_default = parent.size == 0 && parent.version == 0 &&
        parent.kind == NEO_WEBVIEW_NATIVE_PARENT_NONE && parent.handle == nullptr;
...
bool accepts_ui_objects(const neo_webview_app_t* app) noexcept {
    if (!app) return false;
...
void neo_log(neo_webview_app_t* app, neo_webview_log_level_t level, std::string_view category, std::string_view message,
    ...
    try {
        const neo_webview_string_view_t category_view{
            reinterpret_cast<const uint8_t*>(category.data()), static_cast<uint64_t>(category.size())};
        const neo_webview_string_view_t message_view{
...
void neo_complete_app_shutdown(neo_webview_app_t* app) noexcept {
    ...
    neo_log(app, NEO_WEBVIEW_LOG_INFORMATION, "application", "Native application shutdown started");
...
neo_webview_result_t NEO_WEBVIEW_CALL neo_webview_app_create(const neo_webview_app_options_t* options, neo_webview_app_t** output, neo_webview_error_t** error) {
    ...
    try {
        ...
        neo_log(app, NEO_WEBVIEW_LOG_INFORMATION, "application", "Native application initialized");
        *output=app;
        return NEO_WEBVIEW_OK;
...
neo_webview_result_t NEO_WEBVIEW_CALL neo_webview_app_detach(neo_webview_app_t* app, neo_webview_error_t** error) {
    ...
    if (!app->retain()) return neo_fail(error, NEO_WEBVIEW_ERROR_DISPOSED, "application is disposed");
    if (!check_ui(app)) {
        app->release();
        return neo_fail(error, NEO_WEBVIEW_ERROR_WRONG_THREAD, "application detach must run on the owning UI thread");
    }
    const auto state = app->state.load(std::memory_order_acquire);
    if (!app->embedded && state == neo_app_state::running) {
        app->release();
        return neo_fail(error, NEO_WEBVIEW_ERROR_INVALID_STATE, "a running standalone application is shut down by its run loop");
    }
    neo_complete_app_shutdown(app);
    app->release();
    return NEO_WEBVIEW_OK;
...
neo_webview_result_t NEO_WEBVIEW_CALL neo_webview_app_dispatch(neo_webview_app_t* app, neo_webview_dispatch_callback_t callback, void* context) {
    ...
    if(result!=NEO_WEBVIEW_OK){
        if(result==NEO_WEBVIEW_ERROR_INVALID_STATE)neo_log(app,NEO_WEBVIEW_LOG_WARNING,"dispatcher","Native dispatcher queue limit reached");
...
neo_webview_result_t NEO_WEBVIEW_CALL neo_webview_environment_create_view_async(neo_webview_environment_t* env,const neo_webview_view_options_t* options,neo_webview_view_created_callback_t callback,void* context,neo_webview_operation_t** outop,neo_webview_error_t** error){
    if(outop)*outop=nullptr;if(!env||!callback||!valid_struct(options,options?options->size:0,sizeof(*options))||!valid_native_parent(options->parent)||options->decision_timeout_ms>600000||options->bridge_policy>NEO_WEBVIEW_BRIDGE_TRUST_ENTIRE_VIEW||((options->bridge_origin_count!=0)!=(options->bridge_origins!=nullptr))||(options->bridge_policy==NEO_WEBVIEW_BRIDGE_DISABLED&&options->bridge_origin_count!=0)||(options->bridge_policy==NEO_WEBVIEW_BRIDGE_TRUSTED_ORIGINS&&options->bridge_origin_count==0)||(options->bridge_policy==NEO_WEBVIEW_BRIDGE_TRUST_ENTIRE_VIEW&&options->bridge_origin_count!=0))return neo_fail(error,NEO_WEBVIEW_ERROR_INVALID_ARGUMENT,"invalid view arguments");for(uint32_t index=0;index<options->bridge_origin_count;++index)if(!neo_valid_utf8(options->bridge_origins[index])||!valid_origin(neo_string(options->bridge_origins[index])))return neo_fail(error,NEO_WEBVIEW_ERROR_INVALID_ARGUMENT,"invalid bridge origin");if(!check_ui(env->app))return neo_fail(error,NEO_WEBVIEW_ERROR_WRONG_THREAD,"view creation must begin on the UI thread");if(!accepts_ui_objects(env->app))return neo_fail(error,NEO_WEBVIEW_ERROR_DISPOSED,"application shutdown has begun");
...
neo_webview_result_t NEO_WEBVIEW_CALL neo_webview_view_post_message(neo_webview_view_t* v,neo_webview_string_view_t msg,uint32_t json,neo_webview_error_t** e){if(!v||!neo_valid_utf8(msg))return neo_fail(e,NEO_WEBVIEW_ERROR_INVALID_ARGUMENT,"invalid message");if(!check_ui(v->environment->app))return neo_fail(e,NEO_WEBVIEW_ERROR_WRONG_THREAD,"message posting must run on the UI thread");if(!neo_bridge_access_allowed(v,v->source))return neo_fail(e,NEO_WEBVIEW_ERROR_SECURITY,"Web messaging is blocked by the configured bridge policy",0,"bridge");try{return neo_platform_view_post_message(v,neo_string(msg),json!=0,e);}catch(...){return neo_fail(e,NEO_WEBVIEW_ERROR_INVALID_ARGUMENT,"invalid message");}}
