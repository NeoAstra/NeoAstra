...
- verify the private host view binding supplied by trusted host code and keep renderer-generated
  document correlation data explicitly untrusted;
- bind each accepted hello to the current host navigation generation and a fresh host-issued
  document-session ID;
...
Persistent document-start script APIs install one immutable script per view, so the implementation
distinguishes two envelope values rather than presenting a renderer value as trusted. `hostViewBinding`
is generated by the managed host, injected into the private bootstrap closure, and admits an envelope
only to that view. `rendererDocumentId` is generated in each renderer realm and is only an untrusted
correlation value. The host associates it with the current navigation generation only after accepting
that navigation's compatible hello, issues a separate opaque `documentSessionId`, and retains closed
renderer IDs for the view lifetime so replayed hello/application frames cannot establish or target the
replacement session. The renderer identifier is never exposed as or treated as the trusted
...
- [x] Define transport frame schema, feature identifiers, typed errors, and lifecycle state machine.
- [x] Add document-session identity and teardown hooks to the managed/core bridge.
- [x] Implement backend bootstrap adapters and common handshake handling.
- [x] Implement `@neoastra/client` discovery, connection, diagnostics, and close semantics.
- [x] Implement the in-memory testing transport.
- [x] Replace sample/conformance handwritten bridge code.
- [x] Add package build, types, size budget, license, provenance, and publish checks.
- [x] Document migration and backend security differences.
...
### Implementation and verification status (July 26, 2026)

Step 1 source is present for WebView2, WKWebView, and WebKitGTK. The frontend suite executes common
client/mock tests plus browser-object bootstrap fixtures for all three adapters, restrictive-CSP
checks, package/license/provenance/size checks, and vanilla TypeScript, Vite React, and Vite Vue
builds. Managed coordinator tests cover negotiation, limits, excessive duplicate/pre-handshake
frames, timeout, navigation replacement, cross-document hello/application replay, and late frames;
client tests also cover synchronous bootstrap receive-handler registration failure cleanup and typed
rejection. Managed source assertions verify renderer-loss, view-disposal, application-shutdown,
private-handler teardown, and Linux unknown-origin wiring. The Windows native Release build and all
four CTest targets, WebView2 conformance transport, and quick transport benchmarks were executed
locally. The Linux common and WebKitGTK translation units also passed Clang `-fsyntax-only`
compilation under WSL with GTK/WebKitGTK headers.

Runtime transport execution on macOS/WKWebView and Linux/WebKitGTK was not performed on the Windows
development host. A linked Linux native build/test run was also unavailable because the installed WSL
CMake predates the repository's required version, and no macOS compiler or SDK was available, so the
WKWebView translation unit was not compiled locally. Cross-platform native build/test jobs are
configured in CI, but were not executed in this workspace. The adapters are implemented; only the
all-three-backend runtime portion of the exit criterion remains unverified. These limitations are not
evidence of a macOS or Linux native build or runtime pass.
