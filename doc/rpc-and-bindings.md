...
The package defaults `NeoRpcTypeScriptOutput` to `ClientApp/src/generated/neoastra.ts` and puts backend manifest/schema intermediates under `obj`; set the properties explicitly only for nonstandard layouts. Plain JavaScript applications can set `NeoRpcJavaScriptOutput` and, when necessary, `NeoRpcJavaScriptImport` to emit an ESM binding that imports the client from that module specifier. Outputs contain no absolute paths or timestamps. `NeoRpcBaselineManifest` enables compatibility warnings. The generated manifest SHA-256 is suitable for `NeoRpcOptions.ContractHash`.
