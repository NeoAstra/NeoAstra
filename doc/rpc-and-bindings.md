...
The host snapshots trusted view/session identity, source origin, main-frame state, whole-view trust, protocol features, service provider, and contract hash. View/window references are weak and cannot retain disposed UI. Renderer fields cannot replace context identity. After bridge admission, registered operations without a `Permission` are allowed in the trusted local application. An explicitly permissioned operation requires an authorization service and receives `NeoRpcAuthorizationRequest` checks before dispatch; a missing service denies that operation. A capability manifest does **not** filter permissionless registered methods. Use a deliberately restricted registration surface or explicit permissions for restricted views, and no bridge for active untrusted content. See [capabilities and security](capabilities-and-security.md) and the [CodeAlta integration guide](codealta-integration.md).
