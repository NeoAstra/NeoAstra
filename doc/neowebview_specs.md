...
The current Windows, macOS, and Linux byte/file resource-provider contract is synchronous. WebView2 asks for the response during its resource-request callback; the WKWebView backend invokes the same provider and completes its `WKURLSchemeTask` from `startURLSchemeTask`; WebKitGTK completes its registered `WebKitURISchemeRequest` callback directly. Windows consumes file responses as native streams, macOS uses native `NSData` with mapped-if-safe file access, and Linux opens a `GFileInputStream`, so no path copies file contents through managed memory. Linux byte responses are copied into a `GBytes`-backed stream before the managed response lease is released. The later streaming-body contract remains asynchronous because reads and cancellation are genuinely callback-based operations.
